...
def execute_query_test(sql: str, payload: dict, doctypes: List[str] = None) -> Any:
    try:
        if isinstance(payload, str):
            payload = json.loads(payload) if payload else {}

        # ─── INSERT → ORM ─────────────────────────────────────
        if payload and payload.get("operation") == "insert":
            frappe.has_permission(payload["doctype"], "create", throw=True)
            doc = frappe.new_doc(payload["doctype"])
            ...
            return {
                ...
                "doctype": doc.doctype,
                "name": doc.name,
                "message": f"{doc.doctype} created successfully."
            }

        # ─── UPDATE → Raw SQL ──────────────────────────────────
        elif sql and sql.lower().strip().startswith("update"):
            frappe.has_permission(doctypes[0] if doctypes else None, "write", throw=True)
            combined = _build_match_conditions(doctypes)
            if combined:
                sql = _append_conditions(sql, combined)
            frappe.db.sql(sql)
            ...
            # Clear cache so desk reflects updated data immediately
            if doctypes:
                for doctype in doctypes:
                    # Strip "tab" prefix to get the DocType name
                    doctype_name = doctype.replace("tab", "", 1) if doctype.startswith("tab") else doctype
                    frappe.clear_cache(doctype=doctype_name)

            return {"success": True, "operation": "update", "message": "Updated successfully"}

        # ─── DELETE → Raw SQL ──────────────────────────────────
        elif sql and sql.lower().strip().startswith("delete"):
            frappe.has_permission(doctypes[0] if doctypes else None, "delete", throw=True)
            combined = _build_match_conditions(doctypes)
            if combined:
                sql = _append_conditions(sql, combined)
            frappe.db.sql(sql)
            ...
            # Clear cache so desk reflects deletion immediately
            if doctypes:
                for doctype in doctypes:
                    doctype_name = doctype.replace("tab", "", 1) if doctype.startswith("tab") else doctype
                    frappe.clear_cache(doctype=doctype_name)

            return {"success": True, "operation": "delete", "message": "Deleted successfully"}

        # ─── SELECT → Raw SQL ──────────────────────────────────
        elif sql and sql.lower().strip().startswith("select"):
            combined = _build_match_conditions(doctypes)
            if combined:
                sql = _append_conditions(sql, combined)
            return frappe.db.sql(sql, as_dict=True)
            ...
            return {"error": "Invalid request: no sql or payload provided"}
